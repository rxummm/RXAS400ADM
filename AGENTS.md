...
- **表格/树插槽必须显式标注行类型**：`<template #default="{ row }: { row: RowType }">`（el-table）与 `<template #default="{ data }: { data: NodeType }">`（el-tree），**禁止裸 `{ row }` / `{ data }`**（Element Plus 表格列类型与数据不联动，裸插槽时 `row.xxx`/`data.xxx` 全是 any，拼错字段编译期不报）。行类型优先复用 api 层已有接口（`UserVO`/`JobInfo`/`SpoolFile`/`SysMenu` 等）；API 返回类型必须与后端 JSON 形状一致（大写列名如实声明，参照 `api/job.ts` 的 `SpoolFile`/`JobLogRow`）。门禁：`bash scripts/check-frontend-slots.sh`（裸插槽或悬空 RowType 即失败，已接入 frontend CI）；`npm run lint` 内含 CRLF 拼行防护（`node scripts/check-template-join.mjs`，CRLF 文件批量替换必须用含换行的多行 oldString）。**坑**：`SysMenu.id` 等树/表主键字段可能是 `undefined`（`id?: number`），`Set.has(id)`/数组索引前必须守卫 `data.id != null &&`（实测踩过，见 `UserPermDialog.vue`）；无作用域的内容插槽（`<template #default>纯内容</template>`、`#header`/`#footer`/`#empty`）不需要标注类型
...
| `scripts/check-frontend-slots.sh` | 任何带作用域解构的 #default 插槽必须显式标注类型（裸 `{ row }`/`{ data }`/`{ item }` 等即失败）；标注 RowType 必须已 import 或本地声明（R2），内联匿名对象类型合法 | **无**（147/147 已 100% 标注） | frontend.yml `Slot typing gate` | 支持传目标目录参数供回归测试；正则禁用 `$]` 字符类且 `\{` 后不能紧跟 `(`/`[`（见下方环境坑） |
