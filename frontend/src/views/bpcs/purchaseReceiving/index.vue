<template>
  <div class="page-container page-container--fit">
    <div class="search-bar">
      ...
      <el-input v-model="query.pono" class="w-150" :placeholder="$t('bpcs.purchase.pono')" clearable @keyup.enter="forceSearch" />
      <el-input v-model="query.vendor" class="w-150" :placeholder="$t('bpcs.purchase.vendor')" clearable @keyup.enter="forceSearch" />
...
import { searchPurchaseReceiving, BPCS_EXPORT } from '@/api/bpcs'
...
const { tableData, loading, current, size, total, forceSearch } = useSmartQueryTable<Record<string, unknown>>({
  fetchApi: (params) => searchPurchaseReceiving({
    cono: query.cono || '001',
    pono: query.pono || undefined,
    vendor: query.vendor || undefined,
    current: params.current!,
    size: params.size!,
  }) as Promise<{ records: Record<string, unknown>[]; total: number }>,
})
