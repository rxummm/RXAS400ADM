...
const FA_LIST: FaIconItem[] = [
  ...
  { key: 'warning', fullName: 'fa-solid fa-triangle-exclamation', icon: faTriangleExclamation },
...
FA_LIST.forEach((item) => library.add(item.icon))
...
export function resolveIcon(value?: string): ResolvedIcon | null {
  if (!value) return null
  if (isFaIcon(value)) return resolveFaIcon(value) ? { kind: 'fa', value } : null
