...
import com.rxas400adm.email.controller.EmailGroupController;
...
class EmailGroupControllerSecurityTest {
    ...
    void noToken_401() throws Exception {
        mockMvc.perform(get("/api/v1/email/groups"))
    ...
    void hasPermission_200() throws Exception {
        when(emailGroupService.page(1, 10, null))
    ...
    @Test
    @DisplayName("/all 端点需要 EMAIL_SEND 权限")
    @WithMockUser(username = "admin", authorities = "EMAIL_SEND")
    void listAll_needsEmailSend() throws Exception {
        when(emailGroupService.listAll()).thenReturn(List.of());

        mockMvc.perform(get("/api/v1/email/groups/all"))
                .andExpect(status().isOk());
    }
}
