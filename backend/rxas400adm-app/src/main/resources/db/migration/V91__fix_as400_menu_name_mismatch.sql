...
SELECT 'AS400管理', 1, 'as400ManagementGroup', '', '', 'fa-solid fa-server', 3,
    (SELECT id FROM (SELECT id FROM rx_menu WHERE title = 'groupAs400' AND menu_type = 1 LIMIT 1) AS tmp),
    1, 1, 0, NOW(), NOW()
WHERE NOT EXISTS (
...
UPDATE rx_menu m
JOIN (SELECT id FROM rx_menu WHERE menu_name = 'AS400管理' AND menu_type = 1 LIMIT 1) AS new_parent
SET m.parent_id = new_parent.id, m.updated_time = NOW()
WHERE m.menu_name = '用户Profile管理' AND m.menu_type = 1 AND (m.parent_id IS NULL OR m.parent_id = 0);

UPDATE rx_menu m
JOIN (SELECT id FROM rx_menu WHERE menu_name = '用户Profile管理' AND menu_type = 1 LIMIT 1) AS new_parent
SET m.parent_id = new_parent.id, m.updated_time = NOW()
...
UPDATE rx_menu m
JOIN (SELECT id FROM rx_menu WHERE title = 'bpcs' AND menu_type = 1 LIMIT 1) AS new_parent
SET m.parent_id = new_parent.id, m.updated_time = NOW()
WHERE m.menu_name IN ('bpcsControlTower', 'bpcsCpfr', 'bpcsTms')
  AND m.parent_id IN (SELECT id FROM rx_menu WHERE path IN ('/bpcs-forecast', '/bpcs-shipping'));
