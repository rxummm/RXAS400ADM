...
SELECT (SELECT id FROM rx_menu WHERE title = 'bpcs' AND menu_type = 1 LIMIT 1),
       '发票轨迹', 2, 'bpcsInvoice', '/bpcs-invoice', 'views/bpcs/invoice/index.vue', 'BPCS_INVOICE_VIEW', NULL, 5, 1, 1, 0, NOW(), NOW()
WHERE NOT EXISTS (SELECT 1 FROM rx_menu WHERE title = 'bpcsInvoice' AND menu_type = 2);

INSERT INTO rx_menu (parent_id, menu_name, menu_type, title, path, component, perms, icon, sort, visible, status, admin_only, created_time, updated_time)
SELECT (SELECT id FROM rx_menu WHERE title = 'bpcs' AND menu_type = 1 LIMIT 1),
       '销售趋势', 2, 'bpcsSales', '/bpcs-sales', 'views/bpcs/sales/index.vue', 'BPCS_SALES_VIEW', NULL, 6, 1, 1, 0, NOW(), NOW()
WHERE NOT EXISTS (SELECT 1 FROM rx_menu WHERE title = 'bpcsSales' AND menu_type = 2);

INSERT INTO rx_menu (parent_id, menu_name, menu_type, title, path, component, perms, icon, sort, visible, status, admin_only, created_time, updated_time)
SELECT (SELECT id FROM rx_menu WHERE title = 'bpcs' AND menu_type = 1 LIMIT 1),
       '采购订单', 2, 'bpcsPurchase', '/bpcs-purchase', 'views/bpcs/purchase/index.vue', 'BPCS_PURCHASE_VIEW', NULL, 7, 1, 1, 0, NOW(), NOW()
