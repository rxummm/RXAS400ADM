...
-- 权限码
INSERT INTO rx_permission (permission_code, description, module, remark)
SELECT 'DATA_AREA_VIEW', '数据区域查看', 'SYSTEM', '查看 IBM i 数据区域'
WHERE NOT EXISTS (SELECT 1 FROM rx_permission WHERE permission_code = 'DATA_AREA_VIEW');

INSERT INTO rx_permission (permission_code, description, module, remark)
SELECT 'DATA_AREA_EDIT', '数据区域管理', 'SYSTEM', '创建/修改/删除 IBM i 数据区域（CRTDTAARA/CHGDTAARA/DLTDTAARA）'
WHERE NOT EXISTS (SELECT 1 FROM rx_permission WHERE permission_code = 'DATA_AREA_EDIT');

-- 菜单：数据区域（在「系统值」之后）
INSERT INTO rx_menu (parent_id, title, menu_type, name, path, component, perms, icon, sort_order, visible, enabled, is_tab, created_time, updated_time)
SELECT
